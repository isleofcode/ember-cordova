    ...
    return cordovaArgValidator.run(rawArgs, 'platform')
      .then(function(validated) {
        ui.writeLine(chalk.green(
          'Preparing to ' + validated.command + ' platform ' + rawArgs
        ));

        return platformTask.run(validated.command, validated.args, { save: options.save })
          .then(function() {
            //By default we upgrade the default WebView
            //See ${DOCLINK} TODO
            var platform = validated.platform;

            if (validated.command === 'add' && options.defaultWebview !== true &&
